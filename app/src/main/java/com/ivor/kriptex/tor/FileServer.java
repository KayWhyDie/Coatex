...
public class FileServer extends NanoHTTPD {
    ...
    private void makeServerSecure() throws KeyStoreException, IOException, CertificateException, NoSuchAlgorithmException, UnrecoverableKeyException {
        ...
        if (!keyStore.exists()) {
            ...
            // Hidden Service v3 uses ed25519 keys which are not compatible with RSA KeyPair
            // Generate an RSA keypair for the HTTPS keystore instead of using Tor's keys.
            try {
                java.security.KeyPairGenerator kpg = java.security.KeyPairGenerator.getInstance("RSA");
                kpg.initialize(2048);
                java.security.KeyPair rsaKeyPair = kpg.generateKeyPair();
                CryptoUtils.createKeyStore(keyStore.getAbsolutePath(), tor.getID(), rsaKeyPair);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    // Load the keystore we generated in the app's files directory (created above)
    InputStream keystoreStream = new java.io.FileInputStream(keyStore);
    keystore.load(keystoreStream, tor.getID().toCharArray());
    KeyManagerFactory keyManagerFactory = KeyManagerFactory.getInstance(KeyManagerFactory.getDefaultAlgorithm());
    keyManagerFactory.init(keystore, tor.getID().toCharArray());
